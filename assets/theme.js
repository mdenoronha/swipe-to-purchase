...
theme.Collection = (function() {
  ...
  this.selectors = {
    gridDraggable: '.grid-draggable',
    draggable: '.grid-draggable__item',
    draggableMessage: '.grid-draggable__message',
    dropLeft: "#droppable-left",
    dropRight: "#droppable-right",
    variantSelector: '.variant-selector',
    productDraggable: 'data-product-draggable',
    productHandleDraggable: 'data-product-handle',
    variantDraggable: 'data-variant-draggable',
    variantContainer: '.variant-container',
    variantItem: '.variant-container__item',
    undoSkip: '#undo-skip',
  ...
  function Collection(container) {
    this.cloneProductCard = $( selectors.draggable + ':first-of-type')
    this._initDrag()
    this._variantSelect()
    this._removeSkip()
    $(selectors.viewProduct).on('click', this._viewProduct)
    $(selectors.undoSkip).on('click', this._undoSkip.bind(this))

    var self = this;
    $(selectors.dropProduct).on('click', function() {
      var widget = $('.grid-draggable__item:last-of-type').draggable( "widget" );
      var ui = {draggable: widget}
      var addToCartProduct = $(this).attr('id') == 'add-to-cart-product' ? true : false
      self._dropProduct(ui, addToCartProduct)
    })
  }

  Collection.prototype = _.assignIn({}, Collection.prototype, {
    _initDrag: function(skipProducts, collectionId) {

      $( selectors.draggable ).draggable({
        revertDuration: 200,
        revert: true,
        cancel: false
      });
      ...
      $( selectors.dropLeft ).droppable({
        drop: function(event, ui) {
          self._dropProduct(ui, false)
        }
      });

      $( selectors.dropRight ).droppable({
        drop: function(event, ui) {
          self._dropProduct(ui, true)
        }
      });
    },

    _viewProduct: function() {
      var location = $( selectors.draggable + ':last-of-type').attr('data-product-handle')
      window.location.href = '/products/' + location
    },

    _dropProduct: function(ui, addToCart) {
      var self = this;
      var productId = ui.draggable[0].attributes[selectors.productDraggable].value
      var productHandle = ui.draggable[0].attributes[selectors.productHandleDraggable].value 
      $(selectors.draggableButtonsContainer + ' button').attr('disabled', 'disabled')

      $(selectors.draggableMessage).addClass('hide')
      document.cookie = "swiped=swiped"
      ...
      $('.variant-container__item[data-product-selector=' + productId + ']').remove()
      $(ui)[0].draggable[0].remove()
      ...
      if($(selectors.gridDraggable + ' li').length < 3) {
        ...
        $.ajax({
          success: function(){
            $.ajax({
              url: self._setSession(collectionId, 'undoSkip', productHandle),
              success: function() {
                $(selectors.draggableButtonsContainer + ' button').removeAttr('disabled')
              }
          })
        }
      })
    ...
    _variantSelect: function() {
      $( selectors.variantSelector ).change(function() {
        var productId = $(this).data('product-selector')
        var variantId = $(this).val()
        $('.grid-draggable__item[data-product-draggable=' + productId + ']').attr('data-variant-draggable', variantId)
      })
    },

    _retrieveProducts: function() {
      var self = this;

      var pageNext = $('#paginate-next').attr('href')
      console.log(pageNext)

      if(pageNext != 'none') {
        // Check if all product variants are sold out and do not run ajax
        $.ajax({
          type: 'GET',
          url: pageNext,     
          success: function(data) {
            // Product cards
            var productCards = $(data).find('.grid-draggable')
            productCards.find('#droppable-left').remove()
            productCards.find('#droppable-right').remove()
            $(productCards[0].innerHTML).insertAfter('#droppable-left')
            // Variants
            var variantOptions = $(data).find('.variant-container')
            $('.variant-container').prepend($(variantOptions)[0].innerHTML)
            // Next page
            var nextUrl = $(data).find('#next-url').attr('data-next-url')
            nextUrl = !nextUrl ? 'none' : nextUrl
            $('#paginate-next').attr('href', nextUrl)
            self._initDrag()
            self._variantSelect()
          },
          complete: function() {
        ...
        $(selectors.loadingContainer).removeClass('show--opacity').addClass('hide--opacity')
        $(selectors.gridDraggable).removeClass('hide--opacity')
        $(selectors.variantContainer).removeClass('hide--opacity')

        if($(selectors.gridDraggable + ' li').length == 0) {
          $(selectors.draggableButtonsContainer + ' button#undo-skip').removeAttr('disabled')
        } else {
          $(selectors.draggableButtonsContainer + ' button').removeAttr('disabled')
        }
        let cookies = self._parseCookie()
        let swiped = cookies['swiped']
        if(swiped != 'swiped') {
          $(selectors.draggableMessage).removeClass('hide')
        }
      }

      // let draggableData = $('#draggable-data')
      // let limit = parseInt(draggableData.attr('data-limit'))
      // let currentPage = parseInt(draggableData.attr('data-current-page'))
      // let pageNext = currentPage + 1
      // let totalPages = parseInt(draggableData.attr('data-paginate-pages'))

      // if(pageNext < totalPages) {
      //   // Check if all product variants are sold out and do not run ajax
      //   $.ajax({
      //     url:  "/collections/all/products.json?page=" + pageNext + "&limit=" + limit,
      //     type: 'GET'
      //   })
      //   .success(function(data) {
      //     data.products.forEach(function(item, index) {
      //       // First available variant (fix sold out ****)
      //       var firstAvailVariant = 'soldOut'
      //       for(var i = 0; i < item.variants.length; i++) {
      //         if(item.variants[i].available == true) {
      //           var firstAvailVariant = item.variants[i].id
      //           var firstAvailPrice = item.variants[i].price
      //           break;
      //         }
      //       }
      //       self._createVariantSelect(item)
      //       var newEl = self._createProductCard(item, firstAvailVariant, firstAvailPrice)
      //       $(newEl).insertAfter('#droppable-left')
      //       // reinit draggable and variant selector
      //       self._initDrag()
      //       self._variantSelect()
      //       draggableData.attr('data-current-page', pageNext)
      //     })
      //   })
    ...
    _createVariantSelect: function(item) {
      // Variants select
      var optionsArr = []
      item.options.forEach(function(option, index) {
        optionIndex = index + 1
        var result = item.options.find(obj => {
          return obj.position === optionIndex
        })
        var obj = {}
        obj['option' + optionIndex] = result.name
        optionsArr.push(obj)
      })

      var selectEl = $('<div data-product-selector="' + item.id + '" class="variant-container__item"><select class="variant-selector" data-product-selector="' + item.id + '"></select></div>'); 
      item.variants.forEach(function(variant, index) {
        if(variant.title == "Default Title") {
          optionStr = item.title
          selectEl.find('.variant-selector').attr('disabled', 'disabled')
          var disabled = ''
        } else {
          var optionStr = ''
          optionsArr.forEach(function(option, index) {
            let optionIndex = index + 1
            optionStr = optionStr + option['option' + optionIndex] + ': ' + variant['option' + optionIndex].charAt(0).toUpperCase() + variant['option' + optionIndex].slice(1) + ' '
          })
          var disabled = ''
          if(!variant.available) disabled = 'disabled';
        }
        selectEl.find('.variant-selector').append('<option ' + disabled + ' value="' + variant.id + '">' + optionStr + '</option>')
      })

      return selectEl
    },

    _createProductCard: function(item, firstAvailVariant, firstAvailPrice) {
      var el = this.cloneProductCard[0]

      var newEl = $(el).clone()
          .attr('data-variant-draggable', firstAvailVariant)
          .attr('data-product-draggable', item.id)
          .attr('data-product-handle', item.handle)

      $(newEl).find('.hidden-title').text(item.title)
      $(newEl).find('.product-card__title-text').text(item.title)
      if(item.images[0]) {
        var productImage = item.images[0].src ? item.images[0].src : item.featured_image
      }
      $(newEl).find('.product-card__featured-image').css('background-image', 'url("' + productImage + '")')
        .attr('data-bgset', '')
      $(newEl).find('price-item--regular').text(firstAvailPrice)

      return newEl;
    },

    _setSession: function(collectionId, sessionToSave, productId=false) {
      var self = this;
      let cookies = self._parseCookie()
      let skipArr = cookies[sessionToSave + collectionId]

      if(skipArr) {
        if(productId) {
          skipArr.push(productId)
          document.cookie = sessionToSave + collectionId + '=' + JSON.stringify(skipArr)
        }
      } else {
        skipArr = []
        if(productId) {
    ...
    _removeSkip: function() {
      ...
      $.ajax({
         url:self._setSession(collectionId, 'skipProducts'),
         success: function(){
          let cookies = self._parseCookie()
          let skipArr = cookies['skipProducts' + collectionId]

          $( selectors.draggable ).each(function() {
            if(skipArr.includes($(this).data('product-draggable').toString())) {
              var productId = $(this).attr('data-product-draggable')
              $(this).remove()
              $('.variant-container__item[data-product-selector="' + productId + '"]').remove()
            }
          })
    ...
    _undoSkip: function() {
      ...
      let cookies = self._parseCookie()
      let undoSkipArr = cookies['undoSkip' + collectionId]

      if(undoSkipArr.length > 0 && undoSkipArr != undefined) {
        $(selectors.undoSkip).attr('disabled', 'disabled')
        var productHandle = undoSkipArr[undoSkipArr.length - 1]
        ...
        jQuery.getJSON('/products/' + productHandle + '.js', function(data) {
          ...
          for(var i = 0; i < data.variants.length; i++) {
            if(data.variants[i].available == true) {
              var firstAvailVariant = data.variants[i].id
              var firstAvailPrice = data.variants[i].price
              break;
            }
          }
          var newEl = self._createProductCard(data, firstAvailVariant, firstAvailPrice)
          $(newEl).insertBefore('#droppable-right')
          var newVariant = self._createVariantSelect(data)
          ...
          let UndoSkipArr = cookies['undoSkip' + collectionId]
          let undoSkipValueIndex = UndoSkipArr.indexOf(data.handle.toString())
          if (undoSkipValueIndex > -1) {
            UndoSkipArr.splice(undoSkipValueIndex, 1);
          }

          document.cookie = 'undoSkip' + collectionId + '=' + JSON.stringify(UndoSkipArr)

          // sessionStorage.undoSkip = sessionStorage.undoSkip
          ...
          // Remove from cookies
          let skipArr = cookies['skipProducts' + collectionId]
          let valueIndex = skipArr.indexOf(data.id.toString())
          if (valueIndex > -1) {
            skipArr.splice(valueIndex, 1);
          }

          document.cookie = 'skipProducts' + collectionId + '=' + JSON.stringify(skipArr)
        }).done(function() {
          $(selectors.draggableButtonsContainer + ' button').removeAttr('disabled')
        })
      }
